...
var my_nsec = "nsec1cg99f0q7ptvn6w0r2hfns848ses28jqnp40vs4v0s7e67fg88u0sdvqljj"
var peer_npub = "npub1x20uef6xzepy5ygzp92trw27prdy7v3gqxqeed24tlmr5cht5gdsazrq2v"

func go_nostrn():
	if my_nsec == "" or peer_npub == "":
		if $nsec.text == "" or $npub.text == "":
			prints("añade las llaves nsec y/o npub validas",peer_npub ,my_nsec)
			return
		my_nsec = $nsec.text
		peer_npub = $npub.text
	elif $nsec.text != "" or $npub.text != "":
		my_nsec = $nsec.text
		peer_npub = $npub.text
	else:
