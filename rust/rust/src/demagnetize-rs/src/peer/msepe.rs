...
use sha1::digest::generic_array::{GenericArray as Array, typenum::U20};
use std::pin::Pin;
...
pub struct CryptoMethodSet(u32);
...
pub struct HandshakeBuilder<R> {
    pub peer: Peer,
    pub skey: InfoHash,
    pub rng: R,
    pub crypto_provide: CryptoMethodSet,
    pub timeout: Duration,
}

impl<R: Rng> HandshakeBuilder<R> {
    pub fn new(peer: Peer, skey: InfoHash, rng: R) -> Self {
        HandshakeBuilder {
            peer,
            skey,
            rng,
    ...
    */
    ...
    pub fn dh_exchange_timeout(mut self, d: Duration) -> Self {
        self.timeout = d;
        self
    }
    ...
    fn build(self) -> Handshaker {
        ...
        let pada_len = rng.gen_range(0..=512);
        let padc_len = rng.gen_range(0..=512);
        ...
        packet1.extend(rng.sample_iter::<u8, _>(rand::distributions::Standard).take(pada_len));
        Handshaker {
            peer: self.peer,
...
impl fmt::Debug for Keystream {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Keystream::Plaintext => write!(f, "Plaintext"),
            Keystream::Rc4 { .. } => write!(f, "Rc4"),
        }
    }
}
...
fn gen_private_key<R: Rng>(rng: &mut R) -> BigUint {
    ...
    */
    let bytes = rng.sample_iter::<u8, _>(rand::distributions::Standard).take(20).collect::<Vec<_>>();
