...
// impl
pub trait InfoHashProvider: Clone + Send + Sync + fmt::Display {
    fn get_info_hash(&self) -> InfoHash;
}

#[derive(Copy, Clone, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
pub struct InfoHash(pub [u8; InfoHash::LENGTH]);

impl InfoHash {
    pub const LENGTH: usize = 20;

    pub fn from_hex(s: &str) -> Result<InfoHash, InfoHashError> {
        HEXLOWER_PERMISSIVE
            .decode(s.as_bytes())
    ...
    pub fn from_base32(s: &str) -> Result<InfoHash, InfoHashError> {
        BASE32
            .decode(s.as_bytes())
    ...
    pub fn add_query_param(&self, url: &mut Url) {
...
pub struct PeerId(pub [u8; PeerId::LENGTH]);

impl PeerId {
    const LENGTH: usize = 20;

    pub fn generate<R: Rng>(prefix: &str, rng: &mut R) -> PeerId {
...
pub struct PeerIdError(pub usize);
