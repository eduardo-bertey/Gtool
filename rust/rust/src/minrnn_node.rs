...
use xlstm::blocks::minrnn::model::{MinRnnLM, MinRnnLMConfig, MinRnnType};

// Backend for training and inference
...
impl LocalTokenizer {
    ...
    pub fn from_text(text: &str, vocab_size: usize) -> Result<Self, Box<dyn Error>> {
        ...
        let temp_file = "temp_train_minrnn_godot.txt";
...
#[derive(GodotClass)]
#[class(base=Node)]
pub struct MinRNNChat {
    model: Option<MinRnnLM<MyBackend>>,
    tokenizer: Option<LocalTokenizer>,
    device: <MyBackend as Backend>::Device,
    
    #[export] pub rnn_type: GString, // "gru" or "lstm"
    #[export] pub num_layers: i32,
    #[export] pub expansion_factor: i32,
    #[export] pub lr: f64,
...
impl INode for MinRNNChat {
    fn init(base: Base<Node>) -> Self {
        Self {
            ...
            embedding_dim: 256,
            model_file: "minrnn_model.mpk".into(),
            target_vocab_size: 1024,
            tokenizer_path: "minrnn_tokenizer.json".into(),
            seq_length: 256,
...
impl MinRNNChat {
    ...
    pub fn init_session(&mut self, text_file_for_tokenizer: GString) -> bool {
        ...
        let rnn_type = match self.rnn_type.to_string().to_lowercase().as_str() {
            "lstm" => MinRnnType::MinLstm,
            _ => MinRnnType::MinGru,
        };

        let config = MinRnnLMConfig {
            vocab_size,
            embedding_dim: self.embedding_dim as usize,
            num_layers: self.num_layers as usize,
            rnn_type,
            expansion_factor: self.expansion_factor as usize,
        };
        ...
        let model = if Path::new(&m_path).exists() {
            ...
            match recorder.load(m_path.into(), &self.device) {
                Ok(record) => MinRnnLM::init(&config, &self.device).load_record(record),
                Err(_) => {
                    godot_print!("Warning: Failed to load model record, initiating new model.");
                    MinRnnLM::init(&config, &self.device)
                }
            }
        } else {
            MinRnnLM::init(&config, &self.device)
        ...
        godot_print!("MinRNN session initialized as {:?}. Vocab: {}", rnn_type, vocab_size);
        true
    }
    ...
    pub fn generate(&self, seed_text: GString, length: i32) -> GString {
        ...
        let [_, s_len, v_dim]: [usize; 3] = logits.dims();
        ...
        for _ in 0..length {
            ...
            let (logits, next_state) = model.valid().forward(input.valid(), Some(current_state.clone()));
            current_state = next_state;
            
            let [_, _, v]: [usize; 3] = logits.dims();
